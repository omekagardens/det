...
def test_F2_grace_coercion(verbose: bool = True) -> bool:
    ...
    if verbose:
        print(f"  Sentinel a = {sim.a[sentinel]:.4f}")
...
def test_F3_boundary_redundancy(verbose: bool = True) -> bool:
    ...
    def run_scenario(boundary_on: bool):
        ...
        sim = DETCollider1DUnified(params)
        sim.add_packet(25, mass=2.0, width=2.5, momentum=0.4)
        sim.add_packet(75, mass=2.0, width=2.5, momentum=-0.4)
        
        for _ in range(300):
            sim.step()
        
        return np.mean(sim.F[40:60]), sim.total_grace_injected
...
def test_F8_vacuum_momentum(verbose: bool = True) -> bool:
    ...
    sim.pi_R = np.ones_like(sim.pi_R) * 1.0
...
def test_F9_symmetry_drift(verbose: bool = True) -> bool:
    ...
    params = DETParams1D(momentum_enabled=False, gravity_enabled=False, boundary_enabled=False)
    sim = DETCollider1DUnified(params)
    ...
    max_drift = 0
    for _ in range(1000):
        drift = abs(sim.center_of_mass() - initial_com)
...
def run_full_test_suite():
    """Run complete 1D test suite."""
    print("="*70)
    print("DET v6.2 1D COLLIDER UNIFIED - FULL TEST SUITE")

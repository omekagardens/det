...
Operational Improvements (v5.7 - "The Low-Coupling Pivot"):
----------------------------------------------------------
1. Parameter Tuning:
   - Shifted Collider Scan to Low Coupling (g=0.6 - 1.4) based on 1D Rigorous findings.
   - Binding is predicted at g ~ 0.8.
2. Physics:
...
class DETConstants2D:
    def __init__(self, g=0.8): # Default to the new target
        self.g = g
        # Time/Energy
        self.BETA = 2.0 * g
        self.NU = 0.6
...
def run_collider_suite(mode="free"):
    ...
    trials = 10
    # UPDATE: Using low-coupling Goldilocks zone from 1D rigorous results
    g_vals = [0.6, 0.8, 1.0, 1.2, 1.4]

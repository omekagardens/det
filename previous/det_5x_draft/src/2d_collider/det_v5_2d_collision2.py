...
class DETParams:
    ...
    # Angular momentum module (IV.4b) - plaquette spin
    angmom_enabled: bool = True
    alpha_L: float = 0.04           # Spin charging rate from curl(pi)
    lambda_L: float = 0.02          # Spin decay / friction
    mu_L: float = 0.12              # Spin-to-rotational-flux coupling
    L_max: float = 5.0              # Spin clip
...
class DETCollider2D:
    ...
    def __init__(self, params: Optional[DETParams] = None):
        ...
        # Per-plaquette state (IV.4b)
        # L(y,x) represents out-of-plane (z) angular momentum on the XY plaquette whose
        # top-left corner is the node (y,x). Periodic indexing makes boundaries consistent.
        self.L = np.zeros((N, N))
    ...
    def _clip(self):
        ...
        self.L = np.clip(self.L, -self.p.L_max, self.p.L_max)
    ...
    def step(self):
        ...
        # Coordination load H_i should be local and bond-derived.
        # For a 4-neighbor grid we use a canonical local sum proxy:
        #   H_i = σ_i + σ_E + σ_W + σ_S + σ_N
        # This stays strictly local and avoids hidden globals.
        # ============================================================
        H = (self.sigma + E(self.sigma) + W(self.sigma) + S(self.sigma) + Nb(self.sigma))
        ...
        # Conductivity is σ (coherence is already accounted for in the drive interpolation)
        cond_E = self.sigma
        cond_W = self.sigma
        cond_S = self.sigma
        cond_N = self.sigma
        ...
        # Rotational (angular momentum) flux (IV.4b) - divergence-free swirl
        # Uses perpendicular gradient of plaquette spin L. Each edge is shared by two plaquettes,
        # so edge flux is driven by differences of adjacent L values (conservative on the lattice).
        # F-weighted to avoid pushing vacuum.
        if p.angmom_enabled:
            # Edge-average resource (vanishes in vacuum)
            F_avg_E = 0.5 * (self.F + E(self.F))
            F_avg_W = 0.5 * (self.F + W(self.F))
            F_avg_S = 0.5 * (self.F + S(self.F))
            F_avg_N = 0.5 * (self.F + Nb(self.F))

            # Differences of adjacent plaquette spins for each directed edge
            # J_rot_E(y,x) ~ (L(y,x) - L(y-1,x))  (north plaquette difference)
            dL_E = self.L - Nb(self.L)
            dL_W = W(self.L) - Nb(W(self.L))

            # J_rot_S(y,x) ~ -(L(y,x) - L(y,x-1))  (west plaquette difference)
            dL_S = self.L - W(self.L)
            dL_N = Nb(self.L) - W(Nb(self.L))

            J_rot_E = p.mu_L * self.sigma * F_avg_E * dL_E
            J_rot_W = p.mu_L * self.sigma * F_avg_W * dL_W
            J_rot_S = -p.mu_L * self.sigma * F_avg_S * dL_S
            J_rot_N = -p.mu_L * self.sigma * F_avg_N * dL_N
        else:
            J_rot_E = J_rot_W = J_rot_S = J_rot_N = 0
        
        # Total flux per direction
        J_E = J_diff_E + J_mom_E + J_floor_E + J_rot_E
        J_W = J_diff_W + J_mom_W + J_floor_W + J_rot_W
        J_S = J_diff_S + J_mom_S + J_floor_S + J_rot_S
        J_N = J_diff_N + J_mom_N + J_floor_N + J_rot_N
        ...
        # ============================================================
        # STEP 5b: Angular momentum update (IV.4b) - plaquette spin
        # L^+ = (1 - λ_L Δτ_square) L + α_L curl(pi) Δτ_square
        # curl(pi) is the discrete circulation of directed bond momenta around each plaquette.
        # ============================================================
        if p.angmom_enabled:
            # Discrete curl of bond momentum around each plaquette (top-left corner indexing)
            # curl = pi_E(y,x) + pi_S(y,x+1) - pi_E(y+1,x) - pi_S(y,x)
            curl_pi = self.pi_E + E(self.pi_S) - S(self.pi_E) - self.pi_S

            # Plaquette proper time: average of the four corner node clocks
            Delta_tau_sq = 0.25 * (self.Delta_tau + E(self.Delta_tau) + S(self.Delta_tau) + S(E(self.Delta_tau)))

            decay_L = np.maximum(0.0, 1.0 - p.lambda_L * Delta_tau_sq)
            self.L = decay_L * self.L + p.alpha_L * curl_pi * Delta_tau_sq
        
        # ============================================================
        # STEP 6: Structural update (Appendix B - canonical q-locking)
        # q should accumulate on net inward accumulation (compression / retained past): dF > 0
        # q^+ = clip(q + α_q * max(0, dF), 0, 1)
        # ============================================================
        if p.q_enabled:
...
def run_collision_test(params: DETParams, steps: int = 12000, 
    ...
    for t in range(steps):
        ...
        # Peak separation diagnostic: distance between two strongest local maxima
        # (helps distinguish bound dipole vs single merged lump)
        F = sim.F
        # Find local maxima
        from scipy.ndimage import maximum_filter
        thresh = sim.p.F_VAC + 0.3 * (np.max(F) - sim.p.F_VAC)
        local_max = (F == maximum_filter(F, size=5)) & (F > thresh)
        ...
        if len(xs) >= 2:
            ...
            if dy > N/2: dy -= N
            if dy < -N/2: dy += N
            rec['peak_sep'].append(float(np.sqrt(dx*dx + dy*dy)))
        else:
            rec['peak_sep'].append(0.0)
...
def run_q_sensitivity_scan(verbose: bool = True) -> Dict:
 ...
 # ============================================================
 # ADDITIONAL FALSIFIER TESTS FOR ANGULAR MOMENTUM MODULE
 # ============================================================

def test_F10_vacuum_spin(verbose: bool = True) -> bool:
    """
    Falsifier F10: Angular momentum pushes vacuum

    With angmom enabled, initializing L≠0 on plaquettes in a region
    with negligible F≈F_VAC should produce NO sustained transport.
    """
    if verbose:
        print("\n" + "="*60)
        print("TEST F10: Vacuum Spin Falsifier")
        print("="*60)
    ...
    # Nonzero spin pattern
    sim.L = np.random.uniform(-1.0, 1.0, size=sim.L.shape)

    initial_F = np.sum(sim.F)

    max_J_rot = 0.0
    for _ in range(500):
        # Estimate the rotational flux magnitude directly
        E = lambda x: np.roll(x, -1, axis=1)
        ...
        F_avg_E = 0.5 * (sim.F + E(sim.F))
        dL_E = sim.L - Nb(sim.L)
        J_rot_E = params.mu_L * sim.sigma * F_avg_E * dL_E
        max_J_rot = max(max_J_rot, float(np.max(np.abs(J_rot_E))))
    ...
    passed = max_J_rot < 0.01 and drift < 0.01

    if verbose:
        print(f"  Max |J_rot| observed: {max_J_rot:.6f}")
        print(f"  Mass drift: {drift*100:.4f}%")
        print(f"  F10 {'PASSED' if passed else 'FAILED'}: " +
              ("Spin flux vanishes in vacuum" if passed else "Spin incorrectly pushes vacuum!"))

    return passed


def test_F11_rotational_flux_conservative(verbose: bool = True) -> bool:
    """
    Falsifier F11: Rotational flux is conservative (swirl-only)

    With ONLY angular momentum rotational flux active (no diffusion/momentum/floor/phase),
    the transport should not create net sources/sinks and should conserve total mass.
    """
    if verbose:
        print("\n" + "="*60)
        print("TEST F11: Rotational Flux Conservativeness")
        print("="*60)
    ...
    # Disable diffusion drive by zeroing conductivity
    sim.sigma[:] = 1.0

    # Create a localized mass blob and a surrounding spin field
    sim.add_packet((sim.p.N//2, sim.p.N//2), mass=8.0, width=6.0, momentum=(0, 0))
    yy, xx = np.mgrid[0:sim.p.N, 0:sim.p.N]
    cy, cx = sim.p.N//2, sim.p.N//2
    r2 = (xx - cx)**2 + (yy - cy)**2
    sim.L = np.exp(-0.5 * r2 / (10.0**2))  # smooth spin bump

    initial_F = float(np.sum(sim.F))

    # Temporarily turn off other fluxes by forcing their contributions to zero via params
    # (diffusive flux uses a, C, theta, and classical gradients; we set C=1 and theta=0 and a=0)
    sim.theta[:] = 0.0
    sim.C_E[:] = 1.0
    sim.C_S[:] = 1.0
    sim.a[:] = 0.0  # gates diffusion to ~0

    for _ in range(2000):
        sim.step()

    final_F = float(np.sum(sim.F))
    mass_err = abs(final_F - initial_F) / initial_F

    passed = mass_err < 1e-3

    if verbose:
        print(f"  Initial total F: {initial_F:.6f}")
        print(f"  Final total F:   {final_F:.6f}")
        print(f"  Relative mass error: {mass_err*100:.6f}%")
        print(f"  F11 {'PASSED' if passed else 'FAILED'}: " +
              ("Rotational flux conserves mass" if passed else "Rotational flux leaks mass!"))
...
def run_full_test_suite():
    ...
    f10_pass = test_F10_vacuum_spin(verbose=True)
    f11_pass = test_F11_rotational_flux_conservative(verbose=True)
    ...
    print(f"  F10 (Vacuum spin): {'PASS' if f10_pass else 'FAIL'}")
    print(f"  F11 (Rotational conservative): {'PASS' if f11_pass else 'FAIL'}")
...
def create_visualization(rec: Dict, filename: str = 'det_v5_2d_fixed.png'):
    ...
    ax.plot(rec['t'], rec['peak_sep'], lw=1.5)
    ax.set_xlabel('Step')
    ax.set_ylabel('peak-peak sep')
    ax.set_title('Two-Peak Separation (Bound Dipole Diagnostic)')
...
✓ Plaquette angular momentum (IV.4b) → conservative swirl (J_rot)

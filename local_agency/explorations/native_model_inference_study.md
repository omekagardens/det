...
### 13.1 The Sampler is Where You "Stay DET"

If anything must be sacred, it's this:
- Logits are inputs (deterministic)
- The "ur-choice" becomes: choose one token
- That choice is committed as past trace/witness

Build `det_choose_token()` early and make everything flow through it, even when llama.cpp offers built-in sampling. You'll get:
- Reproducible behavior
- A consistent "consequence record"
- Clean separation between "physics of compute" and "truth of choice"

### 13.2 Performance: What Matters Most

**Biggest wins early**:
- KV cache correctness + efficiency (decode speed lives here)
- Batching prefill (prompt ingestion)
- Avoid per-step allocations (same lesson as substrate_lattice.c)
- Keep logits buffer persistent (reused across tokens)

**Biggest trap**:
- Trying to implement matmul/attention as bond diffusion.
  It will be beautiful and slow.

Instead: treat matmul/attention as effect kernels whose output is deterministic and auditable, while the DET runtime controls when/why they run and how results are committed.

### 13.3 Apply substrate_lattice.c Learnings

The lattice physics implementation taught us:
- Pre-allocate all buffers at initialization
- Use SoA layout for GPU coalescing
- Profile before optimizing
- Conservative limiter at bonds, not nodes
- FFT-based solvers where applicable
...
- substrate_lattice.c (performance patterns)

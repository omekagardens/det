...
class DETCollider2D:
    ...
    def step(self):
        ...
        # Presence and proper-time increments (DET clocking)
        # P_i = a_i / (1 + F_i)
        P = self.a / (1.0 + self.F)
        Delta_tau = P * dt

        # Sender-based bond clocks (outflow from i is paid by i's clock)
        Delta_tau_send_E = Delta_tau
        Delta_tau_send_W = Delta_tau
        Delta_tau_send_S = Delta_tau
        Delta_tau_send_N = Delta_tau

        # For convenience in some bond-local updates, also keep symmetric (avg) clocks
        Delta_tau_E_avg = 0.5 * (Delta_tau + E(Delta_tau))
        Delta_tau_S_avg = 0.5 * (Delta_tau + S(Delta_tau))
        ...
        # Agency gate for diffusive flux (bond-local, symmetric)
        gE = np.sqrt(self.a * E(self.a))
        gW = np.sqrt(self.a * W(self.a))
        gS = np.sqrt(self.a * S(self.a))
        gN = np.sqrt(self.a * Nb(self.a))

        J_diff_E = gE * cond_E * drive_E
        J_diff_W = gW * cond_W * drive_W
        J_diff_S = gS * cond_S * drive_S
        J_diff_N = gN * cond_N * drive_N
        ...
        # Conservative limiter (clock-aware): cap total positive outflow per sender clock
        # Units: J is in "per unit sender-time"; actual transferred amount is J * Δτ_sender
        pos_out = (np.maximum(0, J_E) + np.maximum(0, J_W) +
                   np.maximum(0, J_S) + np.maximum(0, J_N))

        # Maximum allowed total amount leaving a cell this step.
        # IMPORTANT: only allow exporting mass ABOVE vacuum, otherwise the post-step
        # clamp to F_VAC would act like a mass source and create drift.
        available = np.maximum(self.F - p.F_VAC, 0.0)
        max_amount_out = p.outflow_limit * available
        amount_out = pos_out * Delta_tau
        scale = np.minimum(1.0, max_amount_out / (amount_out + eps))

        # Scale only positive (outgoing) flows to preserve attractive/returning components
        ...
        # Keep scaled versions of diffusive components for π accumulation
        J_diff_E_scaled = np.where(J_diff_E > 0, J_diff_E * scale, J_diff_E)
        J_diff_S_scaled = np.where(J_diff_S > 0, J_diff_S * scale, J_diff_S)

        # Clocked transport update:
        # Outflow from i uses Δτ_i; inflow to i is neighbor's outflow paid by neighbor's Δτ.
        out_amount = (J_E + J_W + J_S + J_N) * Delta_tau
        in_amount = (W(J_E) * W(Delta_tau) +
                     E(J_W) * E(Delta_tau) +
                     Nb(J_S) * Nb(Delta_tau) +
                     S(J_N) * S(Delta_tau))

        dF_amount = in_amount - out_amount
        self.F = np.clip(self.F + dF_amount, p.F_VAC, 1000)

        # For q accumulation we still want a compression/deficit signal; use clocked net change
        dF = dF_amount
        
        if p.momentum_enabled:
            # π is a bond/transport memory: decay and accumulate on bond-local (avg) clocks
            decay_E = np.maximum(0.0, 1.0 - p.lambda_pi * Delta_tau_E_avg)
            decay_S = np.maximum(0.0, 1.0 - p.lambda_pi * Delta_tau_S_avg)
            self.pi_E = decay_E * self.pi_E + p.alpha_pi * J_diff_E_scaled * Delta_tau_E_avg
            self.pi_S = decay_S * self.pi_S + p.alpha_pi * J_diff_S_scaled * Delta_tau_S_avg
        
        if p.q_enabled:
            # Structural charge accumulates on local compression/deficit (clocked net change)

...
4. **Key fix**: Separate flux limiting for gravitational contribution

Tested parameters:
- KAPPA = 5.0
- GRAV_FLUX_MAX = 0.2  (limits F * v_grav to prevent instability)
- V_VISC = 0.005

Results:
- Collision without gravity: Mass conserved exactly
- Gravity test: Attraction 24 → 7, mass error ~3.5%
"""
...
class DETConstants2D:
    def __init__(self, g=1.2):
        ...
        # Gravitational velocity (KEY PARAMETERS)
        self.GRAV_VEL_VISCOSITY = 0.005
        self.GRAV_FLUX_MAX = 0.2  # Limits F * v_grav
...
class ActiveManifold2D:
    ...
    def step(self):
        ...
        mass_mask = self.F > self.k.F_MASS_THRESHOLD
        ...
        self.C_x += (alpha * np.abs(J_x) - decay_x) * dt
        self.C_y += (alpha * np.abs(J_y) - decay_y) * dt
...
def test_gravity():
    ...
    if traj['sep']:
        ...
        change = final_sep - initial_sep
        final_mass_err = 100 * abs(np.sum(sim.F) - initial_F) / initial_F
        
        print(f"\nInitial: {initial_sep:.1f}, Final: {final_sep:.1f}, Change: {change:+.1f}")
        print(f"Final mass error: {final_mass_err:.1f}%")
        
        if change < -5 and final_mass_err < 10:
...
def run_all_tests():
    ...
    print(f"  GRAV_FLUX_MAX = {k.GRAV_FLUX_MAX}")

...
Implements the four mandatory falsifiers from the structural review:
  ...
  F_QD5 — Energy Coupling:
    ...
    If q decreases in a vacuum (F ≈ 0) region → FAIL

  F_QD6 — Gravitational Stability Under Jubilee:
    A large q compact object undergoing Jubilee must not create nonlocal
    gravitational shifts. Far-field potential must decay smoothly.
    If far-field Φ changes faster than local q changes → FAIL

  F_QD7 — No Spontaneous Annealing:
    Random noise without coherence must not reduce q.
    If q decreases in a low-C, noisy environment → FAIL
  ...
  F_A1+ — Agency Bootstrap Test:
    ...
    If any q decreases with global a=0 → FAIL
...
def test_F_QD5_energy_coupling(verbose=True):
    ...
    (vacuum/depleted). Both have identical q, C, and a. Both have flow.
    ...
    initial_qD_rich = float(np.mean(sim.q[20:80]))
    initial_qD_vacuum = float(np.mean(sim.q[120:180]))
    ...
    final_qD_rich = float(np.mean(sim.q[20:80]))
    final_qD_vacuum = float(np.mean(sim.q[120:180]))
    ...
    if verbose:
        print(f"  Rich region q decrease: {rich_decrease:.6f}")
        print(f"  Vacuum region q decrease: {vacuum_decrease:.6f}")
...
def test_F_QD6_gravitational_stability(verbose=True):
    ...
    def run_sim(jubilee_on, seed=42):
        ...
        return np.array(Phi_trace), sim.Phi.copy(), sim.q.copy()
    ...
    # Test 3: The q difference should be the source of the Φ difference
    # (no amplification — Φ change should be proportional to q change)
    ...
    if verbose:
        ...
        print(f"  Mean |q difference|: {qD_diff:.6f}")
...
def test_F_QD7_no_spontaneous_annealing(verbose=True):
    ...
    high q, moderate agency. Even with flow from noise, the
    ...
    if verbose:
        print(f"  Max q decrease: {max_decrease:.6f}")
...
def test_F_A1plus_agency_bootstrap(verbose=True):
    ...
    If any q decreases OR any a > 0 emerges → FAIL
    """
    if verbose:
        print("\n" + "=" * 60)
    ...
    if verbose:
        print(f"  Max q decrease: {max_qD_decrease:.2e}")
        print(f"  Max agency: {max_agency:.2e}")
        print(f"  Total Jubilee: {total_jubilee:.2e}")
        print(f"  No q decrease: {no_qD_decrease}")

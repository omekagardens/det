"""
DET v7.0 Electromagnetism Module
================================
...
- Strict locality
- Conservation by antisymmetry
- Presence-clocked updates (optional)

Reference: DET Theory Card v6.3
Changelog:
- Presence-clocking support for gravitational coupling
- Agency-gated transport option
- Lattice correction factor for G extraction
- v7 structural-drag compatible presence helper
"""
...
def compute_drag_presence(a: np.ndarray,
                          sigma: np.ndarray,
                          F: np.ndarray,
                          H: np.ndarray,
...
class Electrostatics2D:
    """
    Electrostatic solver: nabla^2 phi = -rho/epsilon, E = -nabla phi
    ...
    def __init__(self, Nx: int = 80, Ny: int = 80, dx: float = 1.0, epsilon: float = 1.0):
        ...
        self.eta_lattice = self._compute_lattice_correction(max(Nx, Ny))

    def _compute_lattice_correction(self, N: int) -> float:
        """Compute lattice correction factor for discrete Laplacian."""
        if N <= 32:
            return 0.901
        elif N <= 64:
            return 0.955
    ...
    def solve(self):
        """Solve Poisson equation with lattice correction."""
        # FFT solve
        rho_k = np.fft.fft2(self.rho)
        # Apply lattice correction
        phi_k = rho_k * self.eta_lattice / (self.epsilon * self.K2)
        phi_k[0, 0] = 0  # Zero mean
        self.phi = np.real(np.fft.ifft2(phi_k))

        # E = -nabla phi (central difference)
    ...
    def energy(self) -> float:
        """Field energy: U = epsilon/2 integral E^2 dA"""
...
class Magnetostatics2D:
    ...
    Solves: nabla^2 Az = -mu Jz, B = nabla x A
    """
...
class Electrodynamics2D:
    ...
    Update equations:
    dEz/dt = (1/epsilon)(dHy/dx - dHx/dy - Jz)
    dHx/dt = -(1/mu) dEz/dy
    dHy/dt = (1/mu) dEz/dx

    v6.3 additions:
    - Presence-clocked updates (optional)
    - Agency-gated transport (optional)
    """

    def __init__(self, Nx: int = 100, Ny: int = 100, dx: float = 1.0,
                 epsilon: float = 1.0, mu: float = 1.0,
                 presence_clocked: bool = False):
        self.Nx, self.Ny = Nx, Ny
        self.dx = dx
        self.epsilon = epsilon
        self.mu = mu
        self.c = 1.0 / np.sqrt(epsilon * mu)
        self.presence_clocked = presence_clocked
        ...
        # Presence field for DET coupling (v6.3)
        self.P = np.ones((Ny, Nx))  # Default: uniform presence
    ...
    def set_presence(self, P: np.ndarray):
        """Set presence field for DET coupling."""
        self.P = P.copy()
    ...
    def step(self):
        ...
        # Get effective time step (presence-clocked if enabled)
        if self.presence_clocked:
            dt_eff = dt * self.P
        else:
            dt_eff = dt
        ...
        # Update E (using presence-clocked dt if enabled)
        # Ez^{n+1} = Ez^n + (dt/eps)(dHy/dx - dHx/dy - Jz)
        dHy_dx = (self.Hy - np.roll(self.Hy, 1, axis=1)) / dx
        dHx_dy = (self.Hx - np.roll(self.Hx, 1, axis=0)) / dx
        curl_H = dHy_dx - dHx_dy

        if self.presence_clocked:
            self.Ez = (1 - self.sigma_e * dt_eff) * self.Ez + (dt_eff / eps) * (curl_H - self.Jz)
        else:
...
def test_coulomb():
    """Test Coulomb law: E proportional to 1/r in 2D."""
    print("\n" + "="*60)
    ...
    print(f"  E(r) proportional to r^{slope:.3f}")
    print(f"  2D Coulomb predicts: r^{-1.0}")
    print(f"  Lattice correction: {sim.eta_lattice:.3f}")
...
def test_biot_savart():
    """Test B proportional to 1/r from long wire."""
    print("\n" + "="*60)
    ...
    print(f"  B(r) proportional to r^{slope:.3f}")
...
def test_wave_speed():
    ...
    print(f"  c = 1/sqrt(epsilon*mu) = {sim.c:.3f}")
...
def test_superposition():
    ...
    print(f"  phi_1 + phi_2 = {phi1 + phi2:.4f}")
    print(f"  phi_both = {phi_both:.4f}")
...
def run_tests():
    """Run all EM tests."""
    print("="*70)
    print("DET v6.3 ELECTROMAGNETISM MODULE - TEST SUITE")
    print("="*70)

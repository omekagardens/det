...
class DETParamsTorch:
    """Unified DET v7.0 parameters for PyTorch backend."""
    N: int = 64
    dim: int = 3
    dt: float = 0.02
    device: str = "cpu"
    ...
    lambda_a: float = 30.0      # Deprecated (kept for compatibility)
    beta_a: float = 0.2
...
class DETColliderTorch:
    ...
    def __init__(self, params: DETParamsTorch):
        ...
        self.last_jubilee = torch.zeros(shape, device=self.device, dtype=torch.float64)
    ...
    def _compute_jubilee(self, D: torch.Tensor) -> torch.Tensor:
        ...
        # Node coherence proxy from local bonds
        C_sum = torch.zeros_like(self.F)
        for d in range(dim):
            C_sum = C_sum + self.C[d] + torch.roll(self.C[d], shifts=1, dims=d)
        C_i = C_sum / (2 * dim)
    ...
    def step(self):
        ...
        # STEP 1-2: Gravity baseline/potential
        self._solve_gravity()

        # STEP 3-4: Presence + proper time
        ...
        # STEP 6: Dissipation (for boundary operators)
        ...
        # Boundary operator: Grace injection
        ...
        # STEP 9: Angular momentum update
        if p.angular_momentum_enabled and self.L is not None and dim >= 2:
        ...
        if p.jubilee_enabled:
            ...
            self.last_jubilee.zero_()

        # STEP 11: Agency update (v7 canonical, no structural ceiling)
        if p.agency_dynamic:
            ...
            if p.epsilon_a > 0:
                xi = torch.clamp(
                    torch.randn_like(self.a) * p.epsilon_a,
                    -p.epsilon_a,
                    p.epsilon_a,
                )
            else:
